...
export const UserApi = (instance: AxiosInstance) => ({
  ...
  async buyDonate({ name, serverName }) {
    const { data } = await instance.post<ResponseUser>('/users/donate', { name, serverName });
  ...
  async changePassword(newPassword: string) {
    const { data } = await instance.patch<ResponseUser>('/users', { newPassword: newPassword });
